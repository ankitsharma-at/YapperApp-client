...
function Chat({ communityId }) {
  ...
  const fetchMessages = async () => {
    try {
      ...
      const response = await axios.get(`${process.env.REACT_APP_SERVER_URL}/api/chat/${communityId}`, {
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`${process.env.REACT_APP_SERVER_URL}/api/chat/${communityId}`, formData, {
        headers: { 
  ...
  const handleDeleteMessage = async (messageId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`${process.env.REACT_APP_SERVER_URL}/api/chat/${messageId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
